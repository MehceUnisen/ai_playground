...
def get_data():
    ...
    csv_file = pd.read_csv('missing_data.csv')
    ...
    # gender[:,0] = label_encoder.fit_transform(csv_file.iloc[:,4])
    ...
    # df_gender = pd.DataFrame(data=gender, index=range(22), columns=['e','k'])
    df_rest = pd.DataFrame(data=rest, index=range(22), columns=['length', 'weight'])

    result_data = pd.concat([df_country, df_rest, df_age, gender], axis=1)
    return result_data